...
public class JdbcConnectionPool implements DataSource{
    ...
    // 定义连接池初始化连接对象的数量
    private static final int INIT_CONNECTION_COUNT = 20;
    ...
    public Connection getConnection() throws SQLException {
        if (connections.size() < 0 ){
            throw  new RuntimeException("连接池已被用完，请稍后连接");
        }
    ...
    /**
     * 装饰着模式 Connection 对象。实现 close 的时候向连接池中归还连接而不是释放与数据库的链接资源
     */
